...
class DinoV2RotationTransformer(nn.Module):
    def __init__(self, output_dim=9, vision_model='facebook/webssl-dino300m-full2b-224', hidden_dim=768, nhead=8, num_layers=2, delta_input_dim=9):
        super().__init__()
        self.processor = AutoImageProcessor.from_pretrained(vision_model, do_rescale=False, size={'height': 224, 'width': 224})
        
        # Force fp32 to match input dtype and avoid mixed precision errors
        self.encoder = AutoModel.from_pretrained(vision_model, torch_dtype=torch.float32)
        ...
        self.regressor = nn.Sequential(
            nn.Linear(hidden_dim, 256), nn.BatchNorm1d(256), nn.ReLU(), nn.Dropout(0.3), nn.Linear(256, output_dim)
        )
    ...
    def forward(self, img1, img2, img3, delta_p1_p2):
        ...
        # Flash Attention commented out to avoid kernel errors
